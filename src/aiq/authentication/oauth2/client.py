...
class OAuth2Client(AuthenticationClientBase):
    ...
    async def authenticate(self, user_id: str | None) -> AuthResult:
        ...
        try:
            authenticated_context: AuthenticatedContext = await auth_callback(self.config,
                                                                              AuthFlowType.OAUTH2_AUTHORIZATION_CODE)
        except Exception as e:
            raise RuntimeError(f"Authentication callback failed: {e}") from e

        auth_result = AuthResult(
            credentials=[BearerTokenCred(token=SecretStr(authenticated_context.metadata.get("access_token")))],
            access_token=authenticated_context.metadata.get("access_token"),
            refresh_token=authenticated_context.metadata.get("refresh_token"),
            token_expires_at=authenticated_context.metadata.get("expires"),
        )
